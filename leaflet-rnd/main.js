...
L.Control.renderer = L.Control.extend({
  options: {
    title: 'Print map',
    position: 'topleft',
    sizeModes: ['Current'],
  ...
  onAdd: function () { 
    ...
    var container = L.DomUtil.create('div', 'hidden leaflet-control-renderer leaflet-bar leaflet-control'); 
    this._addCss();
    if (!this.options.hidden) {
      ...
      var btnClass = 'leaflet-control-renderer-button'
      if (this.options.exportOnly) btnClass = btnClass + '-export'

      this.link = L.DomUtil.create('a', btnClass, container);
      this.link.id = "leafletrenderer";
      this.link.title = this.options.title;
      this.holder = L.DomUtil.create('ul', 'rendererHolder', container);

      this.options.sizeModes.forEach(function (sizeMode) {
        var btn = L.DomUtil.create('li', 'rendererSizeMode', this.holder);
  ...
  printMap: function (event, filename, customSize = null) {
    if (filename) {
    ...
    this._map.fire("renderer-start", { event: event });
    if (!this.options.hidden) {
    ...
    this._createImagePlaceholder(sizeMode, customSize)
  },

  _createImagePlaceholder: function (sizeMode, customSize = null) {
    ...
    domtoimage.toPng(this.mapContainer, {
      ...
      .then(function (dataUrl) {
        ...
        plugin._resizeAndPrintMap(sizeMode, customSize);
      })
  ...
  _resizeAndPrintMap: function (sizeMode, customSize = null) {
    this.outerContainer.style.opacity = 0;
    var pageSize = null;
    if(customSize) {
      pageSize = customSize;
    } else {
      pageSize = this.options.sizeModes.filter(function (item) {
  ...
  _printOpertion: function (sizemode) {
    ...
    domtoimage.toPng(plugin.mapContainer, {
      ...
      .then(function (dataUrl) {
          ...
          plugin._map.fire("renderer-finished");
      })
      .catch(function (error) {
  ...
  _addCss: function () {
    ...
    css.type = "text/css";
    css.innerHTML = `.leaflet-control-renderer-button { 
    ...
    .leaflet-control-renderer-button-export { 
    ...
    .rendererHolder .A4Landscape { 
      transform: rotate(-90deg);
    }

    .leaflet-control-renderer-button{
    ...
    .rendererHolder .rendererSizeMode a {
      border-radius: 0px;
    }

    .rendererHolder .rendererSizeMode:last-child a{
      border-top-right-radius: 2px;
      border-bottom-right-radius: 2px;
      margin-left: -1px;
    }

    .rendererPortrait:hover, .rendererLandscape:hover{
...
L.renderer = function(options) {
  return new L.Control.renderer(options);
};
